...
/** Use gemini-2.5-flash - stable vision model */
async function getModel() {
  const model = genAI.getGenerativeModel({ model: 'gemini-2.5-flash' });
  return { model, modelName: 'gemini-2.5-flash' };
}
...
/** Shared retry helper — calls fn once, retries once on failure */
async function withRetry<T>(fn: () => Promise<T>, label: string): Promise<T> {
  try {
    return await fn();
  } catch (firstError: any) {
    console.warn(`[${label}] First attempt failed, retrying once...`, firstError.message);
    try {
      return await fn();
    } catch (secondError: any) {
      console.error(`[${label}] Retry also failed:`, secondError.message);
      const userMsg =
        secondError.message?.includes('API key') ? 'GEMINI_API_KEY không hợp lệ hoặc chưa được cấu hình'
        : secondError.message?.includes('quota') ? 'Vượt giới hạn quota Gemini API'
        : secondError.message?.includes('404') ? 'Model AI không tồn tại hoặc không khả dụng'
        : secondError.message || 'Không thể tạo đề thi bằng AI';
...
2. Mọi công thức toán học, vật lý, hóa học PHẢI viết bằng LaTeX: inline dùng $...$ và block dùng $$...$$
3. Câu hỏi TNKQ có đúng 4 đáp án (A, B, C, D)
4. Đáp án phải chính xác, có giải thích rõ ràng
5. Điểm mỗi câu phân bổ đều trong mức độ tương ứng
6. TUYỆT ĐỐI KHÔNG dùng các cụm từ sau trong nội dung câu hỏi, đáp án hoặc giải thích: "Theo tài liệu", "Dựa vào tài liệu", "Theo SGK", "Theo bài học", "Theo đoạn văn", "Đoạn trên", "Tài liệu trên", "Văn bản trên", "Trong bài", "Theo bài". Câu hỏi phải hoàn toàn tự nhiên và độc lập.
